...
class WindowTitle(Module):
    ...
    * `{class_name}` - name of application class
    ...
    def get_title(self, conn):
        ...
        # to display it
        if (not self.always_show and (w.border == "normal" or w.type == "workspace" or (p.layout in ("stacked", "tabbed") and len(p.nodes) > 1))):
    ...
    def update_title(self, conn, e):
        ...
        layout_changed = (
            hasattr(e, "binding") and (e.binding.command.startswith("layout") or e.binding.command.startswith("move container") or e.binding.command.startswith("border"))
        )
